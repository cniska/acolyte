...
describe("boundedLiteralCompletenessEvaluator", () => {
  test("returns regenerate when a bounded replace-all literal remains in the edited file", async () => {
    const workspace = await mkdtemp(join(tmpdir(), "acolyte-bounded-literal-"));
    try {
      await mkdir(join(workspace, "src"), { recursive: true });
      await writeFile(
        join(workspace, "src/lifecycle-state.ts"),
        [
          "export function acceptedLifecycleSignal(): void {",
          "  return;",
          "}",
          "",
          "function failureSignatureForError(): void {",
          "  return undefined;",
          "}",
          "",
        ].join("\n"),
      ...
      const session = createSessionContext("task_bound");
      ...
      const ctx = createRunContext({
        ...
        session,
        mode: "work",
        result: { text: "Updated src/lifecycle-state.ts.", toolCalls: [] },
        request: {
          model: "gpt-5-mini",
          message:
            "In src/lifecycle-state.ts, replace each `return undefined;` early exit with a bare `return;`. Update only that file, then stop.",
          history: [],
        },
      });

      const action = boundedLiteralCompletenessEvaluator.evaluate(ctx);
      expect(action.type).toBe("regenerate");
      if (action.type === "regenerate") {
        expect(action.feedback?.summary).toContain("src/lifecycle-state.ts");
        expect(action.feedback?.details).toContain("return undefined;");
      }
    } finally {
  ...
  test("returns done when no requested literal remains", async () => {
    const workspace = await mkdtemp(join(tmpdir(), "acolyte-bounded-literal-"));
    try {
      await mkdir(join(workspace, "src"), { recursive: true });
      await writeFile(
      ...
      expect(boundedLiteralCompletenessEvaluator.evaluate(ctx)).toEqual({ type: "done" });
    } finally {
      await rm(workspace, { recursive: true, force: true });
    }
  });

  test("returns done when the request is not a single-file replace-all literal task", async () => {
    const workspace = await mkdtemp(join(tmpdir(), "acolyte-bounded-literal-"));
    try {
      await mkdir(join(workspace, "src"), { recursive: true });
      await writeFile(join(workspace, "src/lifecycle-state.ts"), "return undefined;\n", "utf8");
      const session = createSessionContext("task_bound");
      recordCall(session, "file-edit", { path: "src/lifecycle-state.ts" });
      const ctx = createRunContext({
        taskId: "task_bound",
        ...
        request: {
          model: "gpt-5-mini",
          message: "In src/lifecycle-state.ts, update the early return and stop.",
          history: [],
        },
      });
  ...
  test("accepts single-quoted literals in the request", async () => {
    const workspace = await mkdtemp(join(tmpdir(), "acolyte-bounded-literal-"));
    try {
      await mkdir(join(workspace, "src"), { recursive: true });
      await writeFile(join(workspace, "src/lifecycle-state.ts"), "return undefined;\n", "utf8");
      ...
      const ctx = createRunContext({
        ...
        request: {
          model: "gpt-5-mini",
          message:
            "In src/lifecycle-state.ts, replace each 'return undefined;' with 'return;'. Update only that file, then stop.",
          history: [],
        },
      });

      expect(boundedLiteralCompletenessEvaluator.evaluate(ctx).type).toBe("regenerate");
    } finally {
      await rm(workspace, { recursive: true, force: true });
    }
  });
