...
function createRunCommandTool(deps: ToolkitDeps, input: ToolkitInput) {
  return createTool({
    ...
    instruction:
      "Use `shell-run` for known repository commands or when the user explicitly asked you to run a command. Do not use it for file read/search/edit fallbacks (`cat`, `head`, `tail`, `nl`, `ls`, `grep`, `sed`, `find`, `rg`, `wc`) — use `file-read`, `file-search`, `file-find`, `file-edit`, or `code-edit`. For explicit named-file bounded tasks, do not run commands after the edit just to double-check the result unless the user asked for verification.",
