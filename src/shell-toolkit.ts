...
function createRunCommandTool(input: ToolkitInput) {
  ...
  return createTool({
    ...
    instruction:
      "Use `run-command` for known repository commands such as documented build/test/verify steps, or when the user explicitly asked you to run a command. Do not guess a verification command just because edits were made, and for explicit 'update file X then stop' tasks, stop after the requested edit unless the user asked for verification. Do not use it for file read/search/edit fallbacks (`cat`, `head`, `tail`, `nl`, `ls`, `grep`, `sed`, `find`, `rg`, `wc`) — use `read-file`, `search-files`, `find-files`, `edit-file`, or `edit-code`.",
