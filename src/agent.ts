...
function buildToolPolicy(baseInstructions: string): string {
  return [
    ...
    "- Keep each finding to one line by default; add a second line only if evidence is not obvious.",
    "- Do not use verbose `Evidence:`/`Recommendation:` sub-bullets unless explicitly requested.",
