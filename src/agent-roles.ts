...
const DEFAULT_ROLE_SOUL: Record<AgentRole, string> = {
  ...
  coder:
    "Role: coder. Focus on practical implementation guidance and compact, execution-oriented responses. Prefer one clear next action; avoid recap/capability sections and avoid lettered choice menus unless explicitly requested. Do not end with confirmation questions unless a risky/destructive step truly needs approval. Do not edit tests unless the user explicitly asks for test changes or a regression fix clearly requires it.",
...
export function buildSubagentContext(role: AgentRole, req: ChatRequest): string {
  ...
  const roleExpectations: Record<AgentRole, string> = {
    ...
    coder:
      "Expected output: practical implementation guidance; use tools when needed and keep results compact; prefer one clear recommendation over option menus; avoid recap/status/capability sections; avoid confirmation questions unless a risky/destructive step needs approval; avoid test edits unless explicitly requested or required for meaningful regression coverage.",
    reviewer:
