...
function createScanCodeTool(deps: ToolkitDeps, input: ToolkitInput) {
  return createTool({
    ...
    instruction: [
      ...
      "For keyword or regex searches, use `file-search`.",
      "Matches include `enclosingSymbol`; use it as `withinSymbol` in follow-up `code-edit`.",
    ].join(" "),
...
function createEditCodeTool(deps: ToolkitDeps, input: ToolkitInput) {
  ...
  return createTool({
    ...
    instruction: [
      "Use `code-edit` for AST-aware refactors and structural rewrites.",
      "Use `file-edit` for plain text edits.",
      "Prefer explicit operation objects.",
      'For identifier renames, use { op: "rename", from: "result", to: "patternResult", withinSymbol: "scanFile" }.',
      "Scoped rename follows symbol kind inside the named scope.",
      "If local/member is ambiguous, set `target` explicitly to `local` or `member`.",
      'Example member rename: { op: "rename", from: "alias", to: "defaultAlias", withinSymbol: "ProviderConfig", target: "member" }.',
      'For broader rewrites, use { op: "replace", rule, replacement, within?, withinSymbol? }.',
      "`rule` may be pattern shorthand, a structured pattern object, or a recursive ast-grep rule object.",
      'Example replace rule: { op: "replace", rule: { all: [{ kind: "call_expression" }, { inside: { pattern: "function $NAME() { $$$BODY }", stopBy: "end" } }] }, replacement: "..." }.',
      "`path` should be a concrete file path, and you should read that file directly before editing.",
      "When the change must stay inside one named helper, declaration, or block, prefer `withinSymbol` with the enclosing name.",
      "If `code-edit` reports no matches, refine scope or rule from current file evidence instead of broadening blindly.",
