...
function createEditCodeTool(deps: ToolkitDeps, input: ToolkitInput) {
  ...
  return createTool({
    ...
    instruction: [
      ...
      "When the same name exists as both a local symbol and a member in that scope, set `target` explicitly to `local` or `member` instead of relying on auto selection.",
