...
function createScanCodeTool(deps: ToolkitDeps, input: ToolkitInput) {
  return createTool({
    ...
    instruction: [
      ...
      "Pass `paths` and `patterns` as arrays.",
...
function createEditCodeTool(deps: ToolkitDeps, input: ToolkitInput) {
  ...
  return createTool({
    ...
    instruction: [
      ...
      "When the same name exists as both a local symbol and a member in that scope, set `target` explicitly to `local` or `member`.",
      ...
      "Use `any` when any one child rule may match, `all` when all child rules must match the same node, `inside` to restrict matches to an ancestor scope, and `has` to require a descendant match.",
      ...
      "`path` must be a concrete file path. Read that file directly right before editing it.",
      ...
      "If that preview shows the requested bounded change, stop immediately.",
