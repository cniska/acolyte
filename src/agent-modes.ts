...
export const agentModes: Record<AgentMode, AgentModeDefinition> = {
  work: {
    ...
    preamble: [
      ...
      "After editing a source file that has a direct test counterpart (e.g. `foo.test.ts` for `foo.ts`), run `test-run` with the test file or source path to validate your changes. `test-run` resolves direct counterparts when available. Do not skip this step.",
  ...
  verify: {
    ...
    preamble: [
      ...
      "After `code-scan` on an edited file, do not `file-read` that same edited file. Use `test-run` or conclude from the evidence you already have.",
      "If targeted tests already ran after the last edit, reuse that evidence in verify mode. Do not rerun the same test files unless you need different coverage.",
      "Use `test-run` for targeted validation when behavior changed and code-scan alone is not enough. Keep tests scoped to the changed test files or direct counterparts. You may pass direct source paths when a counterpart test exists.",
