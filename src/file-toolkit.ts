...
function createEditCodeTool(input: ToolkitInput) {
  ...
  return createTool({
    ...
    instruction:
      "Use `edit-code` for multi-location code changes, rename/refactor updates, or structural rewrites with AST `edits` array. `path` must be a concrete file path (not `.` or a directory), and you should read that file directly right before editing it. Prefer `edit-file` for single-location text edits.",
    inputSchema: z.object({
      path: z.string().min(1),
      edits: z
        .array(
