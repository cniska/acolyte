...
function createReadFileTool(input: ToolkitInput) {
  ...
  return createTool({
    ...
    instruction:
      "Use `read-file` to inspect code before editing. Read whole files by default and use start/end only for very large files. Batch reads while discovering scope; once you are editing named targets, read each target separately right before its edit, then continue directly to `edit-file` or `edit-code`. For one named file with a repeated literal replacement, read it once, compute the edits from that text, and do not re-read the same file unless the edit fails.",
    inputSchema: z.object({
      paths: z
        .array(
          z
