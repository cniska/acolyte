...
- **MEM-1** — Memory persists across sessions in three scopes: session, project, and user. A project is identified by the `owner/repo` its `upstream` remote names, or its `origin` when there is no `upstream`, so every checkout and worktree of one repository shares one project scope however the remote is addressed and whichever forge hosts it, and a fork shares the project it contributes to. A remote addressing a filesystem path names no repository others can share. A workspace whose repository has no remote naming an `owner/repo`, and a workspace that is not in a repository, has no project scope: session and user memory still work there, an explicit project-scoped write fails saying the workspace has no remote, and a background project-scoped commit is skipped and recorded in the trace.
...
- **MEM-15** — User scope is the person, not the machine: signed in, it is the account the cloud token names, so every machine signed into one account reaches one user scope; signed out, every machine uses one local scope. Signing in moves the local scope into the account and reports what moved, dropping a local observation whose content the account already holds. A token that names no account is refused before anything is stored, and a memory the account holds is unreachable while signed out.
