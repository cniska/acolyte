...
- **FR-53** — No credential passes through the browser at sign-in: `login` holds a secret verifier, sends only its hash to the cloud, and trades the code the browser returns — worthless on its own — for the account's tokens. A cloud that returns no code, has no exchange, or refuses one says which, rather than leaving the sign-in to time out.
...
- **AC-22** — A browser sign-in ends signed in without either token appearing in the URL the browser was sent to, and the sign-in still completes with the verifier known only to the CLI. A cloud that returns no code, one that has no exchange route, and one that refuses the exchange are each reported as themselves. (FR-53)
