...
- **PR-10** — Stopping the daemon asks it to shut down before signalling it, and it refuses while any task is running, naming each live task and its session. An explicit force flag stops it regardless. A restart that is refused starts no replacement, and a self-update never forces.
