...
- **FR-49** — Agent Plugins: when enabled, plugins are discovered from `.agents/plugins` in the workspace and in the home directory, each a directory whose manifest declares the supported standard version. A plugin contributes skills and MCP servers from their fixed locations; a plugin whose manifest is invalid or declares an unsupported version is rejected whole, an unknown manifest field is reported and ignored, and a failure confined to one component type leaves the plugin's other components loaded. A plugin's identity is its manifest name, and a workspace plugin shadows a home plugin claiming the same name.
...
- **SEC-9** — Plugins are disabled by default and opt-in per repository, and a plugin's MCP servers additionally require MCP to be enabled. A plugin-declared executable or working directory must resolve inside that plugin's own root or data directory; one that escapes disables the server rather than launching it.
