...
- **MEM-6** — Recall ranks records by relevance combining semantic similarity and keyword overlap; when the query cannot be embedded there is no relevance signal, so recall fails with a classified error naming the cause rather than substituting unranked results.
