...
- **FR-52** — A signed-in machine stays signed in without returning to the browser: Acolyte renews the cloud token from the refresh token `login` stored, before the token expires and once when the cloud rejects one. Only the cloud refusing the refresh token retires it for the run; a renewal that fails for any other reason is attempted again on the next call. A sign-in with no refresh token behind it, and a renewal the cloud refuses, leave the failure to surface as the cloud stated it. `logout` removes the refresh token with the rest.
- **FR-53** — No credential passes through the browser at sign-in: `login` holds a secret verifier, sends only its hash through the browser, and trades the code that comes back — worthless without the verifier — for the account's tokens. Every way that handoff can fail is reported as what it was, never as a timeout.
...
- **FR-34b** — An error that reaches the top of the CLI, from a command or from the chat renderer, prints one line saying what the user can do about it and exits non-zero. The thrown message, any response body it carried, and the stack are shown only when `ACOLYTE_DEBUG=cli` asks for them.
...
- **MEM-8** — Exact-duplicate records are not stored twice within a scope.
...
- **MEM-15** — User scope is the person, not the machine: signed in, it is the account the cloud token names, so every machine signed into one account reaches one user scope; signed out, every machine uses one local scope. Signing in moves the local scope into the account and reports what moved, dropping a local record whose content the account already holds. A token that names no account, and a token the cloud does not accept, is refused before anything is stored, and a memory the account holds is unreachable while signed out.
- **MEM-16** — A scope key is a hash, so only the machine that derived it can say what it names. With cloud sync on, Acolyte tells the account what the current workspace's project is called; a scope nothing has named is shown by its key. Naming is cosmetic and never blocks or fails a request.
...
- **MEM-20** — Every memory record originates in distillation: no tool the model can call, no command, no typed phrase, and no host projection of a file creates one, so a "remember this" message is answered the way any other message is. Restoring from the archive and moving a scope into an account relocate records that distillation already wrote.
...
- **AC-21** — With a stored refresh token, a cloud call made after the API token has expired succeeds and the renewed token is stored, and a call the cloud answers with 401 succeeds on one retry after renewal; with no refresh token, or when the cloud refuses the renewal, the 401 reaches the user as guidance naming `acolyte login`. A renewal answered with 403 or refused by the network is attempted again on the next call. (FR-52, FR-34b)
- **AC-22** — A browser sign-in ends signed in with neither token appearing in any URL the browser was sent to, and the code that does appear there is bound to the verifier the CLI kept back. A handoff that returns no code, a cloud with no exchange route, an exchange the cloud refuses, an unreachable cloud, and a machine that cannot store what it was given are each reported as themselves. (FR-53)
