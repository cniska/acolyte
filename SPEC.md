...
- **TUI-18** — A key sequence, a bracketed paste, or a multi-byte character split across terminal reads produces the single event it was sent as: never literal escape characters in the composer, and never a submit from a newline inside a paste. Held input is always released: a lone escape byte acts as the escape key without waiting for more input, and a sequence or paste whose remainder never arrives is released so that later keystrokes — interrupt included — still reach their handlers.
