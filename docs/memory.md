...
- **Cloud** (feature-flagged): configured via `cloudUrl` + `cloudToken`, backed by Postgres + pgvector. See [Cloud Sync](cloud.md).
...
- `src/memory-store-cloud.ts` — cloud API MemoryStore implementation (feature-flagged)
