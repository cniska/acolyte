...
4. **Refactor**: clean up while green. Improve naming, remove duplication, simplify structure. Run tests (`test-run`) after each change.
...
- `build` for slicing the work each cycle drives
- `test-review` for judging test adequacy after the fact
