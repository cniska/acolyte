...
Skills load from `.agents/skills` in the workspace and in the home directory, and from installed plugins, and answer to `skill:`: typing `/skill:` offers every skill on the roster, and typing a bare skill name offers its prefixed command. A skill and a built-in command of the same name each keep their own address. The `/skills` picker names the scope each one resolved from.
