...
- **render loop** — on each React commit: serialize, diff against last output, and rewrite the active region from its first changed line down, leaving the lines above it untouched. A width change, frozen overflow, static output, or a forced repaint rewrites the whole region instead. Static items flush once to scrollback. When the active region overflows the viewport, top lines are frozen to scrollback and only the bottom portion is re-rendered (see [Frozen Overflow](glossary.md)). Erase and repaint are atomic within a single DEC 2026 synchronized output block to prevent flicker; a commit that also flushes static covers the flush and the repaint in that same block
...
Skills load from `.agents/skills` in the workspace and in the home directory, and from installed plugins, and answer to `skill:`: typing `/skill:` offers every skill on the roster. When nothing starts with what you typed, the menu falls back to the closest matches and scores a skill by its own name too, so `/buld` offers `/skill:build`. A skill and a built-in command of the same name each keep their own address, and a skill's row in the menu carries its own description. The `/skills` picker names the scope each one resolved from.
