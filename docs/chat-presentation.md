...
The scene is cut into identified **sections** (`header`, one per transcript row, `composer`, `footer`). A section is **finalized** when its bytes can never change again. Streaming prose, active tools, pending rows, and mutable geometry are never finalized.
