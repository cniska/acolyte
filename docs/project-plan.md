...
- MVP gate is only marked complete when this milestone is complete.
  Current evidence (February 24, 2026): `dogfood:smoke` now requires two autonomous coding edits when provider is ready, and strict autonomy smoke now fails when provider readiness is missing; `bun run dogfood:gate` is green with recovery/diagnostics/concurrency/delegated checks; `--strict-autonomy` raises delegated thresholds and uses a 3-run stability window (validated from 1/3 to 3/3 in consecutive strict runs).
