...
- Dogfood smoke now requires three real e2e coding edit tasks when provider is ready (including a multi-line structured edit), with an explicit skip when provider credentials are not configured.
