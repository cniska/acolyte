...
5. Errors must follow `docs/errors.md` (coded errors in runtime, normalize at boundaries).
6. Tool after-hooks must receive normalized errors (`message`, `code?`, `kind?`), not raw thrown values.
7. Run `bun run verify` before every commit.
