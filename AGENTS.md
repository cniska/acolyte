...
- New tool → appropriate `*-toolkit.ts`
...
4. Run `bun run verify` before every commit.
5. Changes that affect agent behavior: dogfood with `acolyte run` before merge, not just tests.
6. Do a branch's work in its own git worktree so parallel branches never clobber each other's tree. If the `wt` helper is available, create one with `wt <branch>` (makes the branch + worktree under `.claude/worktrees/<branch>`, runs `scripts/worktree-setup.sh`, opens a tmux window); `wt ls` lists, `cd "$(wt path <branch>)"` enters, `wt rm <branch>` removes after merge. Keep the primary `main` checkout for direct-to-`main` changes only.
